(() => {
    ...
    lanes.forEach((lane, laneIndex) => {
      ...
      for (let i = 0; i < lane.count; i += 1) {
        state.walkers.push({
          ...
          kind: fruits[(laneIndex + i) % fruits.length],
  ...
  function fruitLocalRects(walker) {
    if (walker.kind === "mango") {
      return [
        { x: -16, y: -22, w: 28, h: 4 },
        { x: -22, y: -18, w: 42, h: 8 },
        { x: -26, y: -10, w: 48, h: 24 },
  ...
  function walkerCollisionRects(walker) {
    ...
    return fruitLocalRects(walker).map((box) => ({
      x: walker.x + box.x,
      y: centerY + box.y,
      w: box.w,
      h: box.h,
  ...
  function drawFruit(walker) {
    const x = Math.round(walker.x);
    const y = walker.row * CELL_H + CELL_H / 2;
    const colors = fruitColors(walker.kind);
    const roll = Math.floor(walker.phase * 6) % 4;
    const wobble = roll % 2 === 0 ? -1 : 1;

    ctx.save();
    ctx.translate(x, Math.round(y + wobble));
    rect(-26, 22, 52, 7, "rgba(0, 0, 0, 0.18)");

    fruitLocalRects(walker).forEach((box) => {
      rect(box.x, box.y, box.w, box.h, colors.main);
    });

    if (walker.kind === "mango") {
      rect(-24, -10, 7, 22, colors.shade);
      rect(10, -18, 8, 28, colors.shade);
      rect(-10 + roll * 3, -14, 13, 5, colors.light);
    } else if (walker.kind === "lemon") {
      rect(-26, -6, 8, 16, colors.shade);
      rect(18, -6, 8, 16, colors.shade);
      rect(-8 + roll * 3, -14, 14, 4, colors.light);
    } else {
      rect(-23, -4, 7, 19, colors.shade);
      rect(15, -7, 7, 20, colors.shade);
      rect(-8 + roll * 3, -16, 12, 5, colors.light);
    }

    rect(-3, -27, 7, 6, "#57442f");
    rect(4, -29 + (roll % 2), 13, 6, colors.leaf);
    rect(-15 + roll * 8, 8, 6, 6, colors.shade);
    rect(12 - roll * 5, -2, 5, 5, "#fff5ca");
    ctx.restore();
  }
  ...
  function draw() {
    ...
    drawWorld();
    state.walkers.forEach(drawFruit);
    drawPlayer();
    drawFlash();
    ctx.restore();
  ...
  function wireControls() {
    ...
    document.querySelectorAll("[data-move]").forEach((button) => {
      ...
      button.addEventListener("pointerdown", moveFromButton);
      button.addEventListener("click", moveFromButton);
    });
  }
